            ...
            for (offset, instructions) in state.offset_map {
                for instruction in instructions.instructions() {
                    let instruction = match instruction {
                        ...
                        // TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        Add(0, Value::Const(value)) => MulAdd(offset, 0, *value),
...
mod test {
    ...
    #[rstest(input, expected,
        ...
        case("+++---", [].into()),
        case(">+++<-", [Add(1, 3.into()).into(), Sub(0, Value::Const(1)).into()].into()),
        case(">+++", [Add(1, 3.into()).into(), PtrIncrement(1).into()].into()),
        case("[[[]]]", [Node::Loop([Node::Loop([Node::Loop([].into())].into())].into())].into()),
        case("->+<", [Sub(0, Value::Const(1)).into(), Add(1, 1.into()).into()].into()),
        case("[->>>-<<<]", [Sub(3, Value::Memory(0)).into(), SetValue(0, 0.into()).into()].into()),
        case("+++>-<[->>>-<<<]", [Add(0, 3.into()).into(),Sub(1, Value::Const(1)).into(), Sub(3, Value::Memory(0)).into(), SetValue(0, 0.into()).into()].into()),
        case("[>>>-<<<-]", [Sub(3, Value::Memory(0)).into(), SetValue(0, 0.into()).into()].into()),
        ...
        case("[->+>+>++>+++<<<<]", [Add(1, Value::Memory(0)).into(), Add(2, Value::Memory(0)).into(), MulAdd(3, 0, 2).into(), MulAdd(4, 0, 3).into(), SetValue(0, 0.into()).into()].into()),
